...
def _run_single(jd_text: str, use_summary: bool, truncate_jd_flag: bool,
                client) -> dict:
    """
    运行一次评估，返回 {score, dimensions, tokens, elapsed, parse_ok, raw}。
    不写缓存，不写 tracker，纯粹测量。
    """
    from src.evaluator import build_evaluation_prompt, parse_evaluation_result
    ...
    try:
        ...
        return {"score": 0, "dimensions": {}, "tokens": tokens,
                "elapsed": time.time() - t0, "parse_ok": False,
                "error": str(e), "raw": ""}
    elapsed = time.time() - t0

    result   = parse_evaluation_result(raw, "", "", "", "")
    parse_ok = result["score"] > 0   # 解析成功判断
